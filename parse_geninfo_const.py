...
with open('raw_geninfo_const.txt', 'r', encoding='utf-8') as f:
    text = f.read()

parts = text.split('General Information – Philippine Constitution (Answer Key & Explanations)')
...
current_topic = "Philippine Constitution"

for qb in q_blocks:
    if not qb.strip(): continue
    q_match = re.search(r'^(\d+)\.\s+(.*?)(?=\nA\.)', qb, re.DOTALL | re.MULTILINE)
    ...
    options = []
    opt_matches = re.finditer(r'([A-E])\.\s(.*?)(?=\n[A-E]\.\s|$)', qb[q_match.end():], re.DOTALL)
